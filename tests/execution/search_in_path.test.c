...
START_TEST(test_search_in_path_not_found,
		   "Test that search_in_path returns NULL when the command is not found")
{
	char *path = "/bin:/usr/bin:/usr/local/bin";
...
START_TEST(test_search_in_path_null_path,
		   "Test that search_in_path returns NULL when the path is NULL")
{
	char *path = NULL;
	char *command = "ls";
